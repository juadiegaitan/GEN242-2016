...
Color dots based on grouping information (here 2 groups)
```{r basic_scatter_plot_color, eval=TRUE}
...
- __Step 1__: To get started with this exercise, create a dedicated `workshop` directory and save into this directory the following sample tables. To inspect the content of the files, one can load them in a spreadsheet program or a text editor.
...
Set the working directory of your R session to the `workshop` directory generated in the previous step.
...
Alternative online import of molecular weight table
...
Alternative online import of subcelluar targeting table
...
- __Step 2__: Assign uniform name (here `ID`) to the column containing the gene/transcript IDs
...
- <b><font color="red">Homework 3D</font></b>: How can the merge function in the previous step be executed so that only the common rows among the two data frames are returned? Prove that both methods - the two step version with `na.omit` and your method - return identical results. 
- <b><font color="red">Homework 3E</font></b>: Replace all `NAs` in the data frame `my_mw_target2a` with zeros. Note, for this replacement operation it will be necessary to temporarliy coerce the `data.frame` to a `matrix` using the command `as.matrix`. Subsequently, the `NAs` values can be replaced by zeros with `my_mw_target2a[is.na(my_mw_target2a)] <- 0`. Finally, the resulting matrix can be converted back to a `data.frame` with `as.data.frame`.
...
- __Step 5__: Retrieve all records (rows) with a value of greater than 100,000 in the `MW` column and a 'C' value in the `Loc` column (targeted to chloroplast).
...
- <b><font color="red">Homework 3F</font></b>: How many protein entries in the `my_mw_target` data frame have a MW of greater than 4,000 and less than 5,000. Subset the data frame accordingly and sort it by MW to check that your result is correct.

```{r homework3F_solutions, eval=FALSE, echo=FALSE}
...
- __Step 10__: Generate a scatter plot for the columns named `MW` and `Residues`
...
- __Step 11__: Export the data frame `my_mw_target4` to a tab-delimited text file and inspect it in Excel.

```{r export_example, eval=TRUE}
write.table(my_mw_target4, file="my_file.xls", quote=F, sep="\t", col.names = NA) 
```

## Run Entire Analysis Routine as Script

- <b><font color="red">Homework 3H</font></b>: Write all commands from this exercise to an R script named `exerciseRbasics.R`, or download it from [here](http://faculty.ucr.edu/~tgirke/Documents/R_BioCond/My_R_Scripts/exerciseRbasics.R). Then execute the script with the `source` function as shown below. This will run all commands of this exercise and generate the corresponding output files in the current working directory or your R session.
...
<footer>
 ...
 Site last generated: Apr 9, 2016 <br />
